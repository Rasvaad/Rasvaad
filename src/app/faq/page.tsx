...
import { getFaqs, getSectionMap, getSiteSettings } from "@/lib/sanity";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: sectionSeo?.seoTitle ?? primarySeo?.seoTitle ?? "Catering FAQs | Catering Services in Surat & Navsari",
    description:
    ...
    keywords: sectionSeo?.keywords?.length ? sectionSeo.keywords : faqData?.flatMap((faq) => faq.keywords ?? []) ?? undefined,
    alternates: { canonical: "/faq" },
  };
}

export default async function FAQPage() {
  const [siteSettings, faqData, sections] = await Promise.all([
    getSiteSettings(),
    getFaqs(),
    getSectionMap(),
  ]);
